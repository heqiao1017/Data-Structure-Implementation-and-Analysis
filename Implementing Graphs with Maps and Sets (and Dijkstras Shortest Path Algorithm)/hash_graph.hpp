...
namespace ics {
  ...
  private:
    ...
    public:
      ...
      //  instance variables allows HashGraph access them directly
...
T HashGraph<T>::edge_value(NodeName origin, NodeName destination) const {
  if(!has_edge(origin,destination))
    throw GraphError("HashGraph<T>::edge_value: edge from "+origin+" to "+destination+" does not exist");
...
int HashGraph<T>::in_degree(NodeName node_name) const {
  if(!has_node(node_name))
    throw GraphError("HashGraph<T>::in_degree: "+node_name+" does not exist");
...
int HashGraph<T>::out_degree(NodeName node_name) const {
  if(!has_node(node_name))
    throw GraphError("HashGraph<T>::out_degree: "+node_name+" does not exist");
...
int HashGraph<T>::degree(NodeName node_name) const {
  if(!has_node(node_name))
    throw GraphError("HashGraph<T>::degree: "+node_name+" does not exist");
...
auto HashGraph<T>::out_nodes(NodeName node_name) const -> const NodeSet& {
  if(!has_node(node_name))
    throw GraphError("HashGraph<T>::out_nodes: "+node_name+" does not exist");
...
auto HashGraph<T>::in_nodes(NodeName node_name) const -> const NodeSet& {
  if(!has_node(node_name))
    throw GraphError("HashGraph<T>::in_nodes: "+node_name+" does not exist");
...
auto HashGraph<T>::out_edges(NodeName node_name) const -> const EdgeSet& {
  if(!has_node(node_name))
    throw GraphError("HashGraph<T>::out_edges: "+node_name+" does not exist");
...
auto HashGraph<T>::in_edges(NodeName node_name) const -> const EdgeSet& {
  if(!has_node(node_name))
    throw GraphError("HashGraph<T>::in_edges: "+node_name+" does not exist");
...
void HashGraph<T>::add_edge (NodeName origin, NodeName destination, T value) {
  ...
  edge_values.put(Edge(origin,destination), value);//add to edge_value map

  //update the local info for each node
...
void HashGraph<T>::remove_node (NodeName node_name){
  if(has_node(node_name)){
    EdgeMap edge_value_copy (edge_values);
    for(const auto& edge : edge_value_copy){
      if(edge.first.first== node_name  || edge.first.second== node_name)
        edge_values.erase(edge.first);
    }

    LocalInfo localInfo = node_values.erase(node_name);
    for(const auto& ele : localInfo.out_nodes){
      node_values[ele].in_nodes.erase(node_name);
      node_values[ele].in_edges.erase(Edge(node_name,ele));
    }
    for(const auto& ele : localInfo.in_nodes){
      node_values[ele].out_nodes.erase(node_name);
      node_values[ele].out_edges.erase(Edge(ele,node_name));
    }
  }
}
...
void HashGraph<T>::load (std::ifstream& in_file, std::string separator) {
  ...
  while(getline(in_file,line)){
    ...
    if(line_sp.size()==1){
      add_node(NodeName(line_sp[0]));
    } else{
      origin = line_sp[0];
      destination = line_sp[1];
      iss.str(line_sp[2]);
      iss>>value;
...
bool HashGraph<T>::operator == (const HashGraph<T>& rhs) const{
  ...
  if(node_values.size()!=rhs.node_values.size())
    return false;
  for(const auto& entry : rhs.node_values)
    if(!node_values.has_key(entry.first))
      return false;
...
std::ostream& operator<<(std::ostream& outs, const HashGraph<T>& g) {
  outs<<"graph[";
  HeapPriorityQueue<typename HashGraph<T>::NodeLocalEntry> nodes_sorted(g.node_values
      , [](const typename HashGraph<T>::NodeLocalEntry& a, const typename HashGraph<T>::NodeLocalEntry& b){return a.first<b.first;});
  for(const typename HashGraph<T>::NodeLocalEntry& node : nodes_sorted)
